...
//    fClosingSymbols:array of TClosingSymbolSet;
  ...
  public
    ...
    function AddNewAttribs(Name:String):TSynHighlighterAttributes;
    function AttribsByName(Name:string):TSynHighlighterAttributes;
    function AddAttribs(Attri:TSynHighlighterAttributes):integer;
    procedure DeleteAttribs(Idx:integer); overload;
    procedure DeleteAttribs(Name:string); overload;
...
function TSynRange.AddNewAttribs(Name: String): TSynHighlighterAttributes;
begin
 Result:=TSynHighlighterAttributes.Create(Name);
 fAttribs.Add(Result);
end;

function TSynRange.AttribsByName(Name: string): TSynHighlighterAttributes;
var
 i:integer;
begin
 Result:=nil;
 for i:=0 to fAttribs.Count-1 do
   if TSynHighlighterAttributes(fAttribs[i]).Name=Name then
   begin
...
procedure TSynRange.DeleteAttribs(Name:string);
var
  p:TSynHighlighterAttributes;
begin
  p:=AttribsByName(Name);
  p.Free;
  fAttribs.Remove(p);
end;
...
var
  ...
  function GetNextTag(var Idx:Integer; var TagParam:string; IgnoreUnknown:boolean=false):boolean;
  var
   s:string;
   sPos:PChar;
  begin
  ...
  procedure ReadInfo;
    ...
    begin
      while GetNextTag(CurTagIndex,Param) do
      begin
        case CurTagIndex of
...
begin
  ...
  try
    ...
    if (not GetNextTag(CurTagIndex,Param)) or (CurTagIndex<>xitUniHighlighter) then
...
function TDefaultSymbols.GetToken(parser:TSynUniSyn; var tkSynSymbol:TSynSymbol): boolean;
begin
  inc(parser.Run);
  Result:=False;
end;
...
function TNumberSymbols.GetToken(parser:TSynUniSyn; var tkSynSymbol:TSynSymbol): boolean;
begin
 repeat
   Inc(Parser.Run);
...
function TDefaultTermSymbols.GetToken(parser: TSynUniSyn;
