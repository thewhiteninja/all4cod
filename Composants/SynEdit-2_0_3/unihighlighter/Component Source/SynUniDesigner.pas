...
{$ENDIF}
  ...
  Private
    ...
    RootRange,Range,KeyWords:TTabsheet;
    Memo:TMemo;
    ...
    procedure CreateComponent(out Comp: TControl; CName: CClass; Parent: TWinControl; Align: TAlign);
    procedure DoAddRange(Sender: TObject);
    procedure CreateCheckBox(out comp: TCheckBox; Parent: TWinControl; left, top, width, height: integer);
    procedure CreateLabel(Parent: TWinControl; left, top, width, height: integer; Caption: string);
    procedure CreatePanel(out comp: TPanel; Parent: TWinControl; left, top, width, height: integer);
    ...
    procedure FillTree;
    procedure TreeAddRange(Node: TTreeNode; Range: TSynRange);
  ...
  Public
    ...
    class function EditHighlighter(aHL: TSynUniSyn; Title:string=''): boolean;
...
{$ENDIF}
  ...
  CreateComponent(TControl(PageControl), TPageControl, pProp, alClient);

  CreateComponent(TControl(RootRange), TTabsheet, PageControl, alClient);
  RootRange.PageControl:=PageControl;
  RootRange.TabVisible:=false;
  ...
  EDelimitersRt.OnDblClick:=DelimDblClick;
  ...
  CreateComponent(TControl(Range), TTabsheet, PageControl, alClient);
  Range.PageControl:=PageControl;
  Range.TabVisible:=false;
...
//  EDelimitersRg.font.Charset:=255;
  ...
  EDelimitersRg.OnDblClick:=DelimDblClick;
  ...
  CreateComponent(TControl(KeyWords), TTabsheet, PageControl, alClient);
  KeyWords.PageControl:=PageControl;
  KeyWords.TabVisible:=false;
...
Procedure TSynUniDesigner.CreateCheckBox(out comp:TCheckBox; Parent:TWinControl; left, top, width, height:integer);
begin
...
Procedure TSynUniDesigner.CreateComponent(out Comp:TControl; CName:CClass; Parent:TWinControl; Align:TAlign);
begin
...
Procedure TSynUniDesigner.CreatePanel(out comp:TPanel; Parent:TWinControl; left, top, width, height:integer);
begin
...
class function TSynUniDesigner.EditHighlighter(aHL: TSynUniSyn; Title: string): boolean;
var
  d:TSynUniDesigner;
  iTempSyn: TSynUniSyn;
...
begin
  if (key=vk_Return) and (ssCtrl in Shift) and (PageControl.ActivePage=KeyWords) then
    begin
...
Procedure TSynUniDesigner.TreeAddRange(Node:TTreeNode;Range:TSynRange);
  var temp:TTreeNode;
      i:integer;
begin
  if Node=nil then
    begin
      Temp:=Tree.items.add(nil,Range.name);
      temp.ImageIndex:=0;
      temp.SelectedIndex:=0;
    end
  else
    begin
      Temp:=Tree.items.addChild(Node,Range.name);
      temp.ImageIndex:=1;
      temp.SelectedIndex:=1;
    end;
  Temp.Data:=Range;
  For i:=0 to Range.SymbolGroupCount-1 do TreeAddSymbol(Temp, Range.SymbolGroups[i]);
  For i:=0 to Range.RangeCount-1 do TreeAddRange(Temp, Range.Ranges[i]);
end;
...
begin
  if (key=vk_Delete) and (h.MainRules<>Tree.Selected.Data) and (not Tree.IsEditing) then
    begin
...
begin
 ...
 case GetNodeType(Node) of
   ...
   ntRootRange:begin
       ...
       PageControl.ActivePage:=RootRange;
     end;
   ntKeyWords: begin
       ...
       PageControl.ActivePage:=KeyWords;
     end;
 End;
 UpdatingControls:=false;
end;
...
begin
  if (PageControl.ActivePage=KeyWords) then
      memo.Lines.Add(SampleMemo.SelText);
end;
